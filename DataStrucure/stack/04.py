...
def solution1(data:str) -> str:
    ...
    answer:str = "" # 출력할 값
    s: list[str] = [] # 스택 연산자 값을 저장한다.
    for i in data: # 문자열을 순회 한다. 
        if i.isnumeric(): # 숫자일 경우 answer에 저장 
            answer += i
        elif i in op: # 연산자일 경우. 
            if s and op[i] <= op[s[-1]]: # 연산자가 스택에 마지막 연산자보다 우선 순위가 낮은 경우.
                answer += s.pop() # 스택의 마지막 값을 제거하여 answer에 더한다.
            s.append(i) # 연산자를 스택에 저장한다.


    while s: # 스택에 있는 모든 값을 answer에 저장 한다.
        answer += s.pop()
    return answer
