...
1. name과 yearning을 딕셔너리로 만들어 사용한다. 
2. photo를 반복문을 이용해 이차원 배열을 꺼낸다.
3. 딕셔너리와 매칭 숫가 합산.
4. 없는 값은 0으로 변경
"""
...
#다른 풀이 방법 인덱스를 이용하는 방법
def solution2(name, yearning, photo):
    return [sum(yearning[j] for j in range(len(name)) if name[j] in i) for i in photo]


# [sum(점수[이름.index(j)] for j in i if j in 이름) for i in 사진]
