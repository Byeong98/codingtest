...
section을 처음부터 확인하면서
아직 칠하지 않은 구역을 만나면
그곳을 기준으로 롤러를 배치해 최대한 덮는다
덮은 곳까지는 다시 칠할 필요 없음 → 다음 칠할 곳을 찾는다
"""
def solution(n, m, section):
    answer = 0 # 칠하는 횟수
    last = 0 # 마지막으로 페인트칠한 위치
    for i in section: # 이미 칠한 범위를 넘어서는 경우
        if i > last:
            answer +=1 # 새로운 롤러 사용
            last = i + m -1 # 롤러로 칠할 수 있는 마지막 위치 업데이트
...
def solution2(n, m, section):
    answer = 1  # 최소 한 번은 롤러를 사용해야 함
    prev = section[0]  # 첫 번째 칠해야 하는 구역을 기준으로 시작

    for sec in section:  # section 리스트를 하나씩 확인
        if sec - prev >= m:  # 이전 롤러 범위에서 벗어난 경우
            prev = sec  # 새로운 롤러 시작 위치 업데이트
